...
# Above was just input parsing and bookeeping. Here's where things
...
cp kmp_files rxe_versions ./WORKSPACE/SOURCES
...
# zypper dutifully gathers up the build requirements of that RPM.
    ...
    if [ -n "$ZYPPER_COMMAND" ]; then
        ...
        zypper ar -p 1 --no-gpgcheck ./WORKSPACE/SRPMS rpmbuild-local
        zypper -n source-install -d $NAME
        zypper rr rpmbuild-local
    else
...
echo "rpmbuild -ba ./WORKSPACE/SPECS/$SPECFILE_BASENAME --define \"_topdir $TOPDIR\" --target \"$ARCH\" --define \"_os $OS\" --define \"_arch $ARCH\" --define \"_buildname $BUILDNAME\" --define \"debug_package $DEBUG_PACKAGE\" " > WORKSPACE/helper.sh
